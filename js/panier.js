    ...
    if (localStorage.length > 0) {
        ...
        if (getArrayProductsInLocalStorage()) {
            for (let i = 0; i < getArrayProductsInLocalStorage().length; i++) {
                const id =  getArrayProductsInLocalStorage()[i].id;
...
const getProduct = (id) => {
    ...
    return fetch(`http://localhost:3000/api/cameras/${id}`)
        ...
        .catch(function (error) { console.log('Une erreur est survenue sur la page panier.js => ' + error); });
}

const displayProductOnBasketPage = (article, i) => {
    let lentilles = "", j = 0;
